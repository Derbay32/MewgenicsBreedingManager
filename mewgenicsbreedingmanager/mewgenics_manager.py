...
def _column_labels() -> list[str]:
    return [t(key) for key in COLUMN_LABEL_KEYS] + [_stat_label(s) for s in STAT_NAMES] + [
...
def _stat_label(stat_name: str) -> str:
    return t(STAT_LABEL_KEYS.get(stat_name, stat_name))

def _ability_tip(name: str) -> str:
    """Return a tooltip description for an ability/mutation name, or '' if unknown."""
    key = re.sub(r"[^a-z0-9]", "", name.lower())
    locale_key = f"ability.tip.{key}"
    tip = t(locale_key)
    return "" if tip == locale_key else tip
...
def _read_visual_mutations(raw: bytes) -> list[str]:
    ...
    for i in range(14):
        ...
        if slot_id >= 300:
            name = (
                VISUAL_MUT_NAMES[i] if i < len(VISUAL_MUT_NAMES) else f"Mutation{i + 1}"
            )
...
class Cat:
    ...
    def __init__(
        self, blob: bytes, cat_key: int, house_info: dict, adventure_keys: set
    ):
    ...
    def data(
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if compat == "risky" and not self._show_lineage:
                compat = "ok"
            if col in STAT_COLS:
                base_c = STAT_COLORS.get(
                    cat.base_stats[STAT_NAMES[col - 4]], QColor(100, 100, 115)
                )
                if compat == "incompatible":
                    return QBrush(
                        QColor(
                            base_c.red() // 4, base_c.green() // 4, base_c.blue() // 4
                        )
                    )
                if compat == "risky":
                    return QBrush(
                        QColor(
                            base_c.red() // 2, base_c.green() // 2, base_c.blue() // 2
                        )
                    )
            ...
            if col == COL_STAT:
                ...
                if compat == "incompatible":
                    return QBrush(
                        QColor(sc.red() // 4, sc.green() // 4, sc.blue() // 4)
                    )
                if compat == "risky":
                    return QBrush(
                        QColor(sc.red() // 2, sc.green() // 2, sc.blue() // 2)
                    )
                return QBrush(sc)
            if col == COL_INB:
            ...
            if col in STAT_COLS:
                ...
                return t("tooltip.stat.base_total", stat=_stat_label(n), base=b, delta=extra, total=total_stat)
            if col == COL_ROOM:
                return cat.room
...
_CHIP_STYLE = (
    "QLabel { background:#252545; color:#ccc; border-radius:6px;"
...
class CatDetailPanel(QWidget):
    ...
    def _build_single(self, cat: Cat):
        ...
        if has_mods:
            ...
            for n in STAT_NAMES:
                ...
                if total != base:
                    ...
                    text = f"{_stat_label(n)} {base}{sign}{delta}"
                    col_s = "#5a9" if delta > 0 else "#c55"
                else:
                    text = f"{_stat_label(n)} {base}"
                    col_s = "#555"
                lbl = QLabel(text)
        ...
        if self._show_lineage:
            ...
            tree_btn.clicked.connect(
                lambda: LineageDialog(cat, self, navigate_fn=_navigate).exec()
            )
    ...
    def _build_pair(self, a: Cat, b: Cat):
        ...
        grid.setColumnMinimumWidth(
            0, 110
        )  # ensure label column has room for full names

        # Stat column headers
        for j, stat in enumerate(STAT_NAMES):
            h = QLabel(_stat_label(stat))
...
class LineageDialog(QDialog):
    ...
    def __init__(self, cat: "Cat", parent=None, navigate_fn=None):
        ...
        def make_gen_row(label_text, cat_list, highlight_all=False, dim_all=False):
            ...
            for c in cat_list:
                row.addWidget(
                    cat_box(c, highlight=highlight_all, dim=(dim_all and c is not None))
                )
            row.addStretch()
            outer.addLayout(row)
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self) -> QWidget:
        ...
        self._btn_gone.clicked.connect(lambda: self._filter("__gone__", self._btn_gone))
